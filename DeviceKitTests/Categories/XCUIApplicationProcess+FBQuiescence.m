 ...
 * @brief Implementation of process-level quiescence control with method
 * swizzling.
 *
 * This file implements the FBQuiescence category on XCUIApplicationProcess,
 * providing custom quiescence handling through Objective-C runtime method
 * swizzling.
 *
 * ## Method Swizzling
 *
...
/**
 * Pointer to the original waitForQuiescenceIncludingAnimationsIdle:
 * implementation. Used to call the original behavior after custom timeout
 * configuration.
 */
static void (*original_waitForQuiescenceIncludingAnimationsIdle)(id, SEL, BOOL);

/**
 * Pointer to the original waitForQuiescenceIncludingAnimationsIdle:isPreEvent:
 * implementation. Used on older iOS versions with the two-parameter quiescence
 * method.
 */
static void (*original_waitForQuiescenceIncludingAnimationsIdlePreEvent)(id,
                                                                         SEL,
                                                                         BOOL,
                                                                         BOOL);
 ...
 */
static void
swizzledWaitForQuiescenceIncludingAnimationsIdle(id self, SEL _cmd,
                                                 BOOL includingAnimations) {
 ...
 */
static void swizzledWaitForQuiescenceIncludingAnimationsIdlePreEvent(
    id self, SEL _cmd, BOOL includingAnimations, BOOL isPreEvent) {
    NSString *bundleId = [self bundleID];
    if (![[self fb_shouldWaitForQuiescence] boolValue] ||
        FBConfiguration.waitForIdleTimeout < DBL_EPSILON) {
        [FBLogger logFmt:@"Quiescence checks are disabled for %@ application. "
 ...
 *
 * @note Only one of the two quiescence methods will exist depending on iOS
 * version.
 */
+ (void)load {
    ...
    Method waitForQuiescenceIncludingAnimationsIdlePreEventMethod =
        class_getInstanceMethod(self.class, @selector
                                (waitForQuiescenceIncludingAnimationsIdle:
                                                               isPreEvent:));
    if (nil != waitForQuiescenceIncludingAnimationsIdleMethod) {
        ...
        original_waitForQuiescenceIncludingAnimationsIdle =
            (void (*)(id, SEL, BOOL))method_setImplementation(
                waitForQuiescenceIncludingAnimationsIdleMethod, swizzledImp);
    } else if (nil != waitForQuiescenceIncludingAnimationsIdlePreEventMethod) {
        IMP swizzledImp =
            (IMP)swizzledWaitForQuiescenceIncludingAnimationsIdlePreEvent;
        original_waitForQuiescenceIncludingAnimationsIdlePreEvent =
            (void (*)(id, SEL, BOOL, BOOL))method_setImplementation(
 ...
 * Uses a static char address as a unique key for
...
- (NSNumber *)fb_shouldWaitForQuiescence {
    id result = objc_getAssociatedObject(
        self, &XCUIAPPLICATIONPROCESS_SHOULD_WAIT_FOR_QUIESCENCE);
    if (nil == result) {
        return @(YES);
    }
 ...
 *
 * @param value NSNumber containing YES to enable quiescence checks, NO to
 * disable.
 */
- (void)setFb_shouldWaitForQuiescence:(NSNumber *)value {
    objc_setAssociatedObject(self,
                             &XCUIAPPLICATIONPROCESS_SHOULD_WAIT_FOR_QUIESCENCE,
