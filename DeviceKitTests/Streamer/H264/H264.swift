...
struct H264HTTPHandler: HTTPHandler {
    ...
    func handleRequest(_ request: HTTPRequest) async throws -> HTTPResponse {
        ...
        let bitrate = request.queryInt(
            name: "bitrate",
            default: H264Constants.defaultBitrate,
            min: H264Constants.minBitrate,
            max: H264Constants.maxBitrate
        )
        let quality = Float(request.queryInt(name: "quality", default: H264Constants.defaultQuality, min: 1, max: 100)) / 100.0
        let scalePercent = request.queryInt(
            name: "scale",
            default: H264Constants.defaultScale,
            min: H264Constants.minScale,
            max: H264Constants.maxScale
        )
